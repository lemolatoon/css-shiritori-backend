...
export const startGame = (
  ...
  for (let i = 0; i < userIds.length; i++) {
    ...
    // 例: [A, B, C] -> [B, C, A] -> [C, A, B]
    const rotatedAssignees = [
      ...baseAssignees.slice(i),
      ...baseAssignees.slice(0, i),
    ];
...
export const handleCssSubmission = async (
  ...
  logger.info(
    JSON.stringify({ submission: { userId: user.id, cssLength: css.length } }),
    "CSS submission received",
  );
...
const endTurn = async (
  ...
  // 現在のターンの結果を処理
  const submissionPromises = room.assignments.map(
    async (assignedUserIds, chainIndex) => {
      ...
      const resultImageUrl = await generateScreenshot(
        html,
        css,
        fullOutputPath,
      );
